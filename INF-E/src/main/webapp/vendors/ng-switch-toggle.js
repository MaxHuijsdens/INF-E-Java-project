    ...
    link: function (scope, element, attrs) {
    		scope.$watch('choices', function(choices) {
    			...
    			angular.forEach(choices, function(choice){
    				var inputradio = angular.element($compile('<input id="' + scope.groupname +i+'" name="'+scope.groupname+'" type="radio" value="'+choice.value+'" ng-model="ngModel" ></input>')(scope));
    				var label = angular.element('<label for="' + scope.groupname +i+'" onclick="">'+choice.label+'</label>');
